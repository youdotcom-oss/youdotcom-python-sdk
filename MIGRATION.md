...
The old sub-SDK access patterns (`you.agents.runs.create()`, `you.search.unified()`, `you.contents.generate()`) still work but emit `DeprecationWarning`. They delegate to the new direct methods on the `You` class. Migrate to the direct API at your convenience:
