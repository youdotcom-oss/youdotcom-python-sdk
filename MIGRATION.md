...
`SEARCH_OP_SERVERS`, `CONTENTS_OP_SERVERS`, and the default server URL are `https://ydc-index.io`. No code changes required — the SDK resolves the server automatically.

### Agents API Removed

The `you.agents()` / `you.agents_async()` direct methods and the `you.agents.runs` sub-SDK shim have been removed. The Agents API model classes (`ExpressAgentRunsRequest`, `AdvancedAgentRunsRequest`, `CustomAgentRunsRequest`, `AgentRunsBatchResponse`, etc.) remain in `youdotcom.models` for backward compatibility but the SDK no longer exposes methods to call them. If you need the Agents API, use the REST endpoint directly or a previous SDK version.
...
The sub-SDK layer was a Speakeasy-generated abstraction that added unnecessary indirection — `you.search.unified()` routed through extra layers before reaching the HTTP client. In 2.6.0 the SDK is hand-maintained and these chains are collapsed into direct methods on `You`. The new methods also have simpler signatures: `country`, `language`, `safesearch`, `livecrawl`, and `freshness` accept plain strings (e.g. `"us"`, `"en"`, `"strict"`) and are normalized automatically, so callers no longer need to import enum classes.
