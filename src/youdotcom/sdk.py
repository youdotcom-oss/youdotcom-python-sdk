...
class You(BaseSDK):
    ...
    def __init__(
        ...
        # Keyword-only: these four are new in 3.1.2, so nothing can be relying
        # on their position, and pinning that now means later attribution args
        # can be added or reordered without touching any caller. Matches the
        # generated method surface (``answer``, ``search``, ... are all
        # keyword-only). The nine parameters above stay positional-or-keyword
        # because ``You("api-key")`` is a shape released callers may rely on.
        *,
        ...
        :param app_name: Optional name of the application or integration calling
            the SDK, emitted as the ``client=`` segment of ``X-Client-Info``.
            Must be printable ASCII (excluding ``;`` and ``/``). Defaults to
            ``None`` → segment dropped.
        :param app_version: Optional version paired with ``app_name`` as
            ``client=<name>/<version>``. Requires ``app_name``: passing a
            version without one raises ``ValueError``, since a bare version
            has nowhere to go in the header. Must be printable ASCII
            (excluding ``;`` and ``/``).
        ...
        :param app_url: Optional caller-identity URL for the ``X-Client-Info``
            attribution header. Must be printable ASCII (excluding ``;``).
            Defaults to ``None`` → segment dropped.
        :raises ValueError: If any attribution argument contains non-ASCII
            characters, control characters, or a delimiter (``;`` for all of
            them, plus ``/`` for ``app_name`` / ``app_version``), or if
            ``app_version`` is given without ``app_name``.
        """
        ...
        # Truthiness, not ``is not None``: the header builder drops the
        # segment for any falsy ``app_name``, so ``app_name=""`` with a version
        # -- the shape ``os.getenv("APP_NAME", "")`` produces -- would
        # otherwise pass this guard and then silently emit no ``client=`` at
        # all, losing both values with no error.
        if app_version and not app_name:
            raise ValueError(
    ...
    def answer(
        ...
        *,
        ...
        safesearch: Optional[Union[str, models.SafeSearch]] = None,
