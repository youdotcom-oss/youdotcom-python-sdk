...
This module is hand-maintained (the SDK is no longer generated by Speakeasy).
It mirrors the ``search_post`` request machinery but POSTs to
``/v1/agents/search`` instead of ``/v1/search``. The agents-search endpoint is
a proxy that:
