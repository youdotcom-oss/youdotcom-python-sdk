...
- **Direct methods replacing sub-SDKs**: Added `you.create_run()`, `you.search_unified()`, `you.generate_contents()` (plus async variants) as direct methods on `You`, replacing the `you.agents.runs.create()`, `you.search.unified()`, `you.contents.generate()` sub-SDK paths. The sub-SDK accessors (`you.agents`, `you.search`, `you.contents`) still work but emit `DeprecationWarning` and will be removed in a future major version.
