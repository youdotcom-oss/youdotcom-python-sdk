...
- **Search/Contents host**: `SEARCH_OP_SERVERS` and `CONTENTS_OP_SERVERS` point to `https://ydc-index.io`. The default server URL (used by answer, research, finance research) is `https://api.you.com`. `you.search()` and `you.contents()` now resolve to their own server constants instead of the default. `you.search()` targets `POST /v1/search` (keyed, requires an API key).
...
- **`search()` server_url fix**: `search()` and `search_async()` now use `self._get_url(models.SEARCH_OP_SERVERS[0], None)` (resolving to `ydc-index.io`) instead of a hardcoded server constant, so custom `server_url` passed to `You()` is respected (previously ignored for search only).
