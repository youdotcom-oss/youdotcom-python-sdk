...
class TestFrameCoverageDivergence:
    """`stream_research_task` drops data-less frames; `stream_research` keeps them.

    This is the *actual* remaining reason to prefer the helper now that both
    paths tolerate unenumerated event names, and it was previously documented
    wrongly as a retry difference (stream-open retries are opt-in, so a default
    client performs none). Pinning it here so the documented rationale on four
    surfaces cannot drift away from the code again.
    """

    _FRAMES = (
        b"id: 0\nevent: ping\n\n"  # data-less keep-alive
        b'id: 1\nevent: connected\ndata: {"type":"connected"}\n\n'
    ...
    def _handler(self, request):
        return httpx.Response(
            200, headers={"content-type": "text/event-stream"}, content=self._FRAMES
        )

    def test_generated_method_drops_data_less_frames(self):
        ...
        try:
            with You(
                ...
                with you.stream_research_task(
                    ...
                    seen = [e.event for e in stream]
        finally:
            client.close()
        assert "ping" not in seen
        assert seen == ["connected", "response.done"]

    def test_helper_yields_data_less_frames(self):
        ...
        try:
            with You(
                ...
                seen = [
                    e.event
                    for e in stream_research(
                        you, task_id="00000000-0000-0000-0000-000000000001"
                    )
                ]
        finally:
            client.close()
        assert seen == ["ping", "connected", "response.done"]
