...
class TestResearchAndWaitStreamMode:
    def test_and_wait_stream_returns_completed_detail(self):
        ...
        assert isinstance(detail, TaskDetail)
        assert detail.status.value == "completed"
        # Verify the call sequence: submit is always first; stream and
        # poll follow (in the sync version _open_raw_stream is called
        # before _poll_detail, so the order is deterministic here).
        assert call_log[0] == "submit"
        assert set(call_log[1:]) == {"stream", "poll"}


class TestResearchAndWaitStreamFallback:
    """Tests for stream-mode fallback paths: no terminal event, stream
    error events, and stream-open failures."""

    def test_stream_no_terminal_event_falls_back_to_polling(self):
        """Stream sends connected but no terminal event; function falls
        back to polling and returns the completed TaskDetail."""
        import json
        ...
        def handler(request):
            ...
            if url.endswith("/stream") or "/stream?" in url:
                call_log.append("stream")
                # Only connected, no terminal event - stream ends cleanly
                chunks = [
                    b'id: 0\nevent: connected\ndata: {"type":"connected","task_id":"abc","status":"running"}\n\n',
                ]
                return httpx.Response(
        ...
        assert detail.status.value == "completed"
        assert call_log[0] == "submit"
        assert "stream" in call_log
        assert "poll" in call_log

    def test_stream_error_event_raises_runtime_error(self):
        """Stream sends an error event; function raises RuntimeError
        with the event name in the message."""
        import json
        ...
        def handler(request):
            ...
            if url.endswith("/stream") or "/stream?" in url:
                chunks = [
                    b'id: 0\nevent: connected\ndata: {"type":"connected","task_id":"abc","status":"running"}\n\n',
                    b'id: 1\nevent: error\ndata: {"type":"error","task_id":"abc","status":"failed"}\n\n',
                ]
                return httpx.Response(
                    200,
                    headers={"content-type": "text/event-stream"},
            ...
            raise AssertionError("Should not poll after stream error event")
        ...
        with pytest.raises(RuntimeError, match="non-completed state: error"):
            research_and_wait(
                you,
                mode="stream",
                interval_s=0.01,
    ...
    def test_stream_open_failure_falls_back_to_polling(self):
        """Stream endpoint returns 500; function falls back to polling
        instead of orphaning the task."""
        import json
...
class TestResearchAndWaitStreamModeAsync:
    ...
    async def test_async_and_wait_stream_returns_completed_detail(self):
        ...
        assert isinstance(detail, TaskDetail)
        assert detail.status.value == "completed"
        # Sequential stream-then-poll: the SSE stream is consumed first, then
        # a single poll fetches the final TaskDetail. Verify both were called
        # and that submit came first.
        assert call_log[0] == "submit"
        assert set(call_log[1:]) == {"stream", "poll"}


class TestResearchAndWaitStreamFallbackAsync:
    """Async tests for stream-mode fallback paths."""

    @pytest.mark.asyncio
    async def test_async_stream_no_terminal_event_falls_back_to_polling(self):
        """Stream sends connected but no terminal event; async function
        falls back to polling and returns the completed TaskDetail."""
        import json
    ...
    @pytest.mark.asyncio
    async def test_async_stream_error_event_raises_runtime_error(self):
        """Stream sends an error event; async function raises RuntimeError."""
        import json
    ...
    @pytest.mark.asyncio
    async def test_async_stream_open_failure_falls_back_to_polling(self):
        """Stream endpoint returns 500; async function falls back to polling."""
        import json
