...
- **Unit tests** (`tests/test_search.py`, `tests/test_contents.py`, `tests/test_answer.py`, `tests/test_direct_methods.py`, `tests/test_shims.py`): Run against MockTransport or the Go mockserver in `tests/mockserver/`.
...
The OpenAPI specs for search and contents use `https://ydc-index.io` as the server URL.
...
# Both should show "https://ydc-index.io"
```

The base `SERVERS` in `src/youdotcom/sdkconfiguration.py` should also remain `https://ydc-index.io` (used by all endpoints: search, contents, research, finance_research).
...
- `src/youdotcom/research_helpers.py` — background-mode helpers (`research_background`, `poll_research_task`, `research_and_wait`, `stream_research`)
- `src/youdotcom/_shims.py` — backward-compat sub-SDK shims (`SearchShim`, `ContentsShim`)
- `src/youdotcom/errors/paymentrequired_response_error.py` — hand-written 402 error class

If `speakeasy run` overwrites or deletes these, restore them from git (`git checkout HEAD -- <file>`).

**Verify User-Agent**: `__user_agent__` in `_version.py` is derived from `__version__` at runtime. `BaseSDK._build_request` sets it directly on each request. No hook is needed.

```bash
# Verify __user_agent__ is derived from __version__
grep "__user_agent__" src/youdotcom/_version.py
# Should show: __user_agent__: str = f"youdotcom-python-sdk/{__version__}"
```
