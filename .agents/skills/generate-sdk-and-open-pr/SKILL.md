...
> **Note (v2.6.0+):** The SDK is no longer generated by Speakeasy. Steps 1-4 below (Speakeasy generation, `speakeasy bump`, `speakeasy run`) are deprecated. The SDK is now hand-maintained — to release a new version, skip to step 5 (version bump, changelog, tests, commit, push, PR). The Speakeasy workflow steps are retained for reference but should not be followed as-is.
...
The base `SERVERS` in `src/youdotcom/sdkconfiguration.py` should remain `https://api.you.com` (used by answer, research, finance_research, get_research_task, stream_research_task). Search and contents use their own `SEARCH_OP_SERVERS`/`CONTENTS_OP_SERVERS` constants (`https://ydc-index.io`) explicitly via `self._get_url(models.SEARCH_OP_SERVERS[0], None)` / `self._get_url(models.CONTENTS_OP_SERVERS[0], None)`.
